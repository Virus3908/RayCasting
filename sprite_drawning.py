...
class Sprites:
    def __init__(self):
        self.sprite_parameters = {
            'sprite_barrel' : {
                    'sprite' : pygame.image.load(path.join(sprite_dir, 'barrel/base/0.png')).convert_alpha(),
                    'viewing_angle' : None,
                    'shift': 1.8,
                    'scale': (0.4, 0.4),
                    'side' : 30,
                    'animation': deque(
                        [pygame.image.load(path.join(sprite_dir, f'barrel/anim/{i}.png')).convert_alpha() for i in range(12)]),
                    'death_animation': deque(
                        [pygame.image.load(path.join(sprite_dir, f'barrel/death/{i}.png')).convert_alpha()
                         for i in range(4)]),
                    'is_dead' : None,
                    'dead_shift' : 2.6,
                    'animation_dist': 800,
            ...
            'sprite_pin' : {
                    'sprite' : pygame.image.load(path.join(sprite_dir, 'pin/base/0.png')).convert_alpha(),
                    'viewing_angle' : None,
                    'shift': 0.6,
                    'scale': (0.6, 0.6),
                    'side' : 30,
                    'animation': deque(
                        [pygame.image.load(path.join(sprite_dir, f'pin/anim/{i}.png')).convert_alpha()
                         for i in range(8)]),
                    'death_animation': [],
                    'is_dead' : 'immortal',
                    'dead_shift' : None,
            ...
            'sprite_flame' : {
                    'sprite' : pygame.image.load(path.join(sprite_dir, 'flame/base/0.png')).convert_alpha(),
                    'viewing_angle' : None,
                    'shift': 0.7,
                    'scale': (0.6, 0.6),
                    'side' : 30,
                    'animation': deque(
                        [pygame.image.load(path.join(sprite_dir, f'flame/anim/{i}.png')).convert_alpha()
                         for i in range(16)]),
                    'death_animation': [],
                    'is_dead' : 'immortal',
                    'dead_shift' : None,
            ...
            'npc_devil' :{
                    'sprite' : [
pygame.image.load(path.join(sprite_dir, f'devil/base/{i}.png')).convert_alpha() for i in range(8)],
                    'viewing_angle' : True,
                    'shift': 0.0,
                    'scale': (1.1,1.1),
                    'side' : 50,
                    'animation': [],
                    'death_animation': deque([pygame.image.load(path.join(sprite_dir,
                                                                          f'devil/death/{i}.png')).convert_alpha()
                                              for i in range(6)]),
                    'is_dead': None,
                    'dead_shift': 0.6,
                    'animation_dist': None,
                    ...
                    'flag': 'npc',
                    'obj_action': deque(
                        [pygame.image.load(path.join(sprite_dir, f'devil/anim/{i}.png')).convert_alpha()
                         for i in range(9)]),
                },
            'sprite_door_h': {
                'sprite' : [pygame.image.load(path.join(sprite_dir,
                                                        f'doors/door_h/{i}.png')).convert_alpha()
                            for i in range(16)],
                'viewing_angle' : True,
                'shift': 0.1,
                'scale': (2.6, 1.2),
            ...
            'sprite_door_v': {
                'sprite' : [pygame.image.load(path.join(sprite_dir,
                                                        f'doors/door_v/{i}.png')).convert_alpha()
                            for i in range(16)],
                'viewing_angle' : True,
                'shift': 0.1,
                'scale': (2.6, 1.2),
            ...
            'npc_soldier0': {
                'sprite': [pygame.image.load(path.join(sprite_dir,
                                                       f'npc/soldier0/base/{i}.png')).convert_alpha()
                           for i in range(8)],
                'viewing_angle' : True,
                'shift' : 0.8,
                'scale' : (0.4, 0.6),
                ...
                'animation' : [],
                'death_animation' : deque([pygame.image.load(path.join(sprite_dir,
                                                       f'npc/soldier0/death/{i}.png')).convert_alpha()
                           for i in range(10)]),
                'is_dead' : None,
                'dead_shift' : 1.7,
                'animation_dist' : None,
                ...
                'blocked' : True,
                'flag' : 'npc',
                'obj_action' : deque([pygame.image.load(path.join(sprite_dir,
                                                       f'npc/soldier0/action/{i}.png')).convert_alpha()
                           for i in range(4)]),
                }
        ...
        self.list_of_objects = [
            ...
            SpriteObject(self.sprite_parameters['sprite_flame'], (8.6, 5.7)),
            SpriteObject(self.sprite_parameters['sprite_door_h'], (3.5, 3.5)),
            SpriteObject(self.sprite_parameters['sprite_door_v'], (1.5, 4.5)),
...
class SpriteObject:
    def __init__(self, parameters, pos):
        ...
        self.viewing_angle = parameters['viewing_angle']
        ...
        if self.viewing_angle:
            if len(self.object) == 8:
                self.sprite_angle = [frozenset(range(338,361)) | frozenset(range(0,23))] + \
                                    [frozenset(range(i,i+45)) for i in range(23, 338, 45)]
            else:
                self.sprite_angle = [frozenset(range(348,361)) | frozenset(range(0,11))] + \
                                    [frozenset(range(i,i+23)) for i in range(11, 348, 23)]
            
            
            self.sprite_positions = {angle: pos for angle, pos in zip(self.sprite_angle, self.object)}
    @property
    def is_on_fire(self):
    ...
    def object_locate(self, player, walls):
        ...
        self.gamma = self.theta - player.angle
        if (dx > 0 and 100<= math.degrees(player.angle) <=360 or
        dx<0 and dy<0):
            self.gamma += DOUBLE_PI
        self.theta -= 1.4 * self.gamma

        delta_rays = int(self.gamma / DELTA_ANGLE)
        ...
        if self.viewing_angle:
            ...
            for angle in self.sprite_angle: #
                if self.theta in angle: #
                    return self.sprite_positions[angle] #
        return self.object
