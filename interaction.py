...
class Interaction:
    def __init__(self, player, sprites, drawing):
        ...
        self.pain_sound = pygame.mixer.Sound(path.join(snd_dir, 'pain.wav'))
    ...
    def clear_world(self):
        ...
        [self.sprites.list_of_objects.remove(obj) for obj in deleted_objects if
         obj.delete]
    ...
    def check_win(self):
        if not len([obj for obj in self.sprites.list_of_objects if obj.flag == 'npc'
                    and not obj.is_dead]):
            pygame.mixer.music.stop()
            pygame.mixer.music.load(path.join(snd_dir, 'win.mp3'))
